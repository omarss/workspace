...
export class TwilioWhatsAppProvider implements ChannelProvider {
  ...
  supports(channel: Channel): boolean {
    return channel === Channel.WHATSAPP_TEXT;
  }
  ...
  async send(
    ...
    const to = request.mobileNumber.startsWith('whatsapp:')
      ? request.mobileNumber
      : `whatsapp:${request.mobileNumber}`;

    const callbackUrl = overrideCallback ?? this.buildCallbackUrl(baseUrl, request.id);
    ...
    if (!response.ok) {
      ...
      throw new Error(
        `Twilio WhatsApp error ${response.status}: ${text.slice(0, 200)}`,
      );
    }
  ...
  private buildCallbackUrl(baseUrl: string | undefined, requestId: string) {
    ...
    const url = new URL('/webhooks/twilio/whatsapp/status', baseUrl);

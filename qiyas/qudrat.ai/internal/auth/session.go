...
// ErrSessionNotFound if revoked, expired, or unknown. On success the
// session row's last_seen_at is updated as a best-effort write — failure
// to touch does not fail the lookup.
func (s *SessionService) Lookup(ctx context.Context, token string) (LookupResult, error) {
	...
	_ = s.store.TouchSessionLastSeen(ctx, sess.ID)

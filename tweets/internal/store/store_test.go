...
func TestSaveBatch_RoundTrip(t *testing.T) {
	...
	got, err := db.Latest(ctx, []server.Country{server.CountryKSA}, nil, nil, time.Time{}, 10)
	if err != nil {
...
func TestSaveBatch_Upsert(t *testing.T) {
	...
	got, _ := db.Latest(ctx, []server.Country{server.CountryKSA}, nil, nil, time.Time{}, 10)
	if len(got) != 1 {
...
func TestLatest_PerCountryIsolation(t *testing.T) {
	...
	ksa, _ := db.Latest(ctx, []server.Country{server.CountryKSA}, nil, nil, time.Time{}, 10)
	eg, _ := db.Latest(ctx, []server.Country{server.CountryEgypt}, nil, nil, time.Time{}, 10)
...
func TestLatest_DefaultLimitWhenZero(t *testing.T) {
	db := newTestDB(t)
	ctx := context.Background()
	got, err := db.Latest(ctx, []server.Country{server.CountryKSA}, nil, nil, time.Time{}, 0)
	if err != nil {
		t.Fatalf("latest: %v", err)
	}
...
func TestPurgeOlderThan(t *testing.T) {
	...
	remaining, _ := db.Latest(ctx, []server.Country{server.CountryKSA}, nil, nil, time.Time{}, 10)

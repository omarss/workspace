...
func (f *FixtureSource) Feed(_ context.Context, req FeedRequest) (FeedResult, error) {
	...
	// Apply keyword query when set. nil and Always both mean "no
	// filter" — the latter is what query.Parse returns for empty
	// input, so honor it explicitly.
	if req.QueryExpr != nil {
		_, alwaysTrue := req.QueryExpr.(query.Always)
		if !alwaysTrue {
			filtered := out[:0]
			for _, tw := range out {
				if req.QueryExpr.Matches(strings.ToLower(tw.Text)) {
					filtered = append(filtered, tw)
				}
			}
			out = filtered

"""Tests for the WhatsApp bot database layer."""
...
_tmp.close()
os.environ["SWET_WHATSAPP_ACCOUNT_SID"] = "test-sid"
os.environ["SWET_WHATSAPP_AUTH_TOKEN"] = "test-token"
os.environ["SWET_WHATSAPP_PHONE_NUMBER"] = "whatsapp:+1234567890"
os.environ["SWET_WHATSAPP_DB_PATH"] = _tmp.name
os.environ["ANTHROPIC_API_KEY"] = "test-key"

from swet_whatsapp.db import (  # noqa: E402
    get_or_create_user,
    get_user_bookmarks,
...
def _cleanup():
    ...
    yield
    try:
        os.unlink(os.environ["SWET_WHATSAPP_DB_PATH"])
    except FileNotFoundError:
        pass
    _tmp2 = tempfile.NamedTemporaryFile(suffix=".db", delete=False)
    _tmp2.close()
    os.environ["SWET_WHATSAPP_DB_PATH"] = _tmp2.name
    # Reset config cache
    import swet_whatsapp.config

    swet_whatsapp.config._config = None
...
def test_get_or_create_user_creates():
    user_id = get_or_create_user("+1555123456", display_name="Test")
    assert user_id == "+1555123456"


def test_get_or_create_user_idempotent():
    uid1 = get_or_create_user("+1555123456", display_name="Test")
    uid2 = get_or_create_user("+1555123456", display_name="Updated")
    assert uid1 == uid2 == "+1555123456"
...
def test_save_and_get_preferences():
    user_id = get_or_create_user("+1555000001", display_name="Test")
    save_user_preferences(
        user_id=user_id,
...
def test_save_and_get_question():
    ...
    from swet_whatsapp.db import get_user_question
...
def test_user_isolation():
    ...
    user1 = get_or_create_user("+1555000001", display_name="User1")
    user2 = get_or_create_user("+1555000002", display_name="User2")

...
from swet_slack.formatters import (  # noqa: E402
    MAX_MESSAGE_LENGTH,
    _split_message,
...
def test_split_long_message():
    # Create a message longer than MAX_MESSAGE_LENGTH (3000 for Slack)
...
def test_format_question_mcq():
    ...
    assert len(parts) >= 1
    assert "Test Question" in parts[0]
    assert "SWET Question" in parts[0]
...
def test_mrkdwn_escaping():
    """Question text with special characters should be escaped for Slack."""
    question = {
        "competency_slug": "programming_fundamentals",
        "difficulty": 3,
        "format": "short_answer",

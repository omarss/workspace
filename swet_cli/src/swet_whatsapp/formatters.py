"""WhatsApp message formatting for questions, grades, stats, etc.

Uses WhatsApp's limited markdown: *bold*, _italic_, ~strikethrough~,
`code`, ```code blocks```. No HTML — plain text with WhatsApp markdown.
...
# MCQ option labels for text-based selection
_MCQ_LABELS = ["A", "B", "C", "D", "E", "F"]
...
def _escape(text: str) -> str:
    """Minimal escaping for WhatsApp markdown.

    WhatsApp uses *bold*, _italic_, ~strike~, `code`, ```blocks```.
    We pass through most text as-is since user/generated content
    rarely conflicts with these markers in practice.
    """
    return text
...
def format_question(question: dict) -> list[str]:
    ...
    MCQ options are rendered inline as A/B/C/D text since WhatsApp buttons
    are limited to 3 options.
    """
    ...
    lines = [
        "*SWET Question*",
        f"*{comp_name}* | {diff_label} | {fmt_label}{est_time}",
        "",
        f"*{question['title']}*",
        "",
        question["body"],
    ...
    if question.get("code_snippet"):
        lines.append("")
        lines.append(f"```{question['code_snippet']}```")

    # MCQ options rendered as inline text
    if question["format"] == "mcq" and question.get("options"):
        lines.append("")
        for i, option in enumerate(question["options"]):
            if i < len(_MCQ_LABELS):
                label = _MCQ_LABELS[i]
                lines.append(f"{label}. {option}")
        lines.append("")
        lines.append("_Reply with A, B, C, or D_")
...
def format_grade(grade: GradeResult, question: dict, time_seconds: float | None = None) -> str:
    """Format the grading result for WhatsApp display."""
    ...
    if is_mcq:
        ...
        if grade.criteria_scores:
            ...
            for cs in grade.criteria_scores:
                lines.append(f"{_BULLET} {cs.name}: {cs.score}/{cs.max_points}")
                if cs.feedback:
                    lines.append(f"  _{cs.feedback}_")
...
def format_level_progress(competency_slug: str, old_level: int, new_level: int) -> str:
    ...
    if new_level > old_level:
        ...
        return f"*Level up! {comp_name}: L{old_level} -> {new_label}*"
    new_label = DIFFICULTY_LABELS.get(new_level, f"L{new_level}")
    return f"{comp_name} adjusted to {new_label}"


def format_stats(stats: list[dict], streak: int | None = None, longest_streak: int | None = None) -> str:
    """Format aggregate stats for WhatsApp display."""
    lines: list[str] = []
...
def format_history(history: list[dict]) -> str:
    """Format recent attempt history for WhatsApp display."""
    if not history:
        return "No attempts yet. Send /q to get started."
    ...
    for i, attempt in enumerate(history, 1):
        ...
        lines.append(
            f"{i}. *{title}*\n   {comp_name} | {fmt_label} | L{attempt['difficulty']} | {score_str}{time_str}{date_str}"
        )
...
def format_preferences(prefs: dict) -> str:
    ...
    lines = [
        "*Your Preferences*",
        "",
        f"{_BULLET} *Roles:* {', '.join(role_names) or 'none'}",
        f"{_BULLET} *Languages:* {', '.join(prefs['languages']) or 'none'}",
        f"{_BULLET} *Frameworks:* {', '.join(prefs['frameworks']) or 'none'}",
        f"{_BULLET} *Question Types:* {formats_str}",
        f"{_BULLET} *Question Length:* {length_display.get(question_length, question_length.title())}",
    ]
...
def format_assessment_results(results: dict[str, dict]) -> str:
    ...
    for slug, data in results.items():
        ...
        lines.append(f"{_BULLET} *{comp_name}*: {level_label} (confidence: {confidence:.0%})")
        if distribution:
            lines.append(f"  _{distribution}_")

    lines.append("")
    lines.append("_These levels will adapt as you practice. Send /test anytime to reassess._")
...
def format_session_summary(results: list[dict]) -> str:
    ...
    for i, result in enumerate(results, 1):
        ...
        lines.append(f"{i}. *{title}* | {comp_name} | {score_str}{time_str}")
...
def format_bookmarks(bookmarks: list[dict]) -> str:
    ...
    for bm in bookmarks:
        ...
        lines.append(
            f"{_BULLET} *{bm['title'][:40]}*\n"
            f"  {comp_name} | {fmt_label} | L{bm['difficulty']}" + (f" | {date_str}" if date_str else "")
        )
...
def format_competencies(competency_levels: dict[str, dict], all_slugs: list[str]) -> str:
    ...
    for i, slug in enumerate(all_slugs, 1):
        ...
        lines.append(f"{i}. *{comp_name}* | Level: {level_str} | Attempts: {attempts_str}")

"""Slack bot database: user-scoped schema for multi-user isolation.

Manages its own SQLite database file separate from the CLI, API, and Telegram bot.
All assessment tables include a user_id column scoped to Slack user IDs (e.g. U0ABC123).
"""
...
_SCHEMA = """
-- Slack users (user_id as primary key, e.g. U0ABC123)
...
def get_db() -> sqlite3.Connection:
    """Open (or create) the Slack bot database and ensure schema exists."""
    db_path = get_slack_config().db_path
...
def get_or_create_user(user_id: str, username: str | None = None, display_name: str | None = None) -> str:
    """Get or create a Slack user. Returns user_id (already a string like U0ABC123)."""
    conn = get_db()
...
def clear_user_state(user_id: str, prefix: str) -> None:
    """Clear all state keys matching a prefix for a user."""
    conn = get_db()
    conn.execute(
        "DELETE FROM state WHERE user_id = ? AND key LIKE ?",
        (user_id, f"{prefix}%"),
    )
    conn.commit()
    conn.close()

"""Telegram bot database: user-scoped schema for multi-user isolation.

Manages its own SQLite database file separate from the CLI and API.
All assessment tables include a user_id column scoped to Telegram chat IDs.
"""
...
_SCHEMA = """
-- Telegram users (chat_id as primary key)
...
def get_db() -> sqlite3.Connection:
    """Open (or create) the Telegram bot database and ensure schema exists."""
    db_path = get_telegram_config().db_path
...
def get_or_create_user(chat_id: int, username: str | None = None, first_name: str | None = None) -> str:
    """Get or create a Telegram user. Returns user_id (str(chat_id))."""
    user_id = str(chat_id)
    conn = get_db()

...
func (c *Client) SendOTP(ctx context.Context, to, code string) error {
	body := sendRequest{
		From:    c.from,
		To:      []string{to},
		Subject: "Your prompter code",
		Text:    fmt.Sprintf("Your prompter sign-in code is %s. It expires in 10 minutes.", code),
	}
	...
	// Try to surface the API error message; fall back to status text.

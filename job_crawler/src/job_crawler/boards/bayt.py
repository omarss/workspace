...
class BaytCrawler(BoardCrawler):
    ...
    def parse(self, raw: RawPosting) -> ParsedPosting | None:
        ...
        return ParsedPosting(
            ...
            # Keep whatever currency Bayt's JSON-LD reports — typically
            # USD for SA postings. Normalisation to SAR (USD * 3.75)
            # happens centrally in `to_upsert` so every source benefits
            # from the same rule. The earlier hard "SAR" override was
            # wrong: Bayt's numbers ARE USD-denominated (just often
            # placeholder tier-signal ranges); blindly relabeling kept
            # the magnitudes wrong.
